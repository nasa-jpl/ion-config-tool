...
// Automatically extracted from source file ../../editor/src/IonModelLoader.jsx
function extractModel(modelObj) {
    ...
    if(modelObj.hasOwnProperty("ionModelName"))
      ion.name = modelObj["ionModelName"];
       ...
       setError("The json file is not an Ion Model.");
       return false;
    }
    ...
    debug("ion IS:  " + JSON.stringify(ion));
    var nodeList = [];
    ...
    for (i=0; i<graphList.length; i++) {  // normally just one item
      ...
      debug("graph item=" + JSON.stringify(graphObj) );
      configsObj = graphObj.configs;
      debug("graph configs=" + JSON.stringify(configsObj) );
    ...
    for (i=0; i<contactsList.length; i++) {  // normally just one item
      ...
      debug("graph item=" + JSON.stringify(graphObj) );
      configsObj = graphObj.configs;
      debug("contacts configs=" + JSON.stringify(configsObj) );
      const configKeyList = extractConfigs(graphKey,configsObj);
      debug("Node got configKeys: " + configKeyList);
      graphs[graphKey].configKey = configKeyList[0];   // assuming just one
    }
...
function extractCommands(groupKey,configType,configKey,commandsList) {
  // extract JSON config definition & build config with commands & cloneValues
    debug("extractCommands groupKey:" + groupKey);
    debug("extractCommands configType:" + configType);
    debug("extractCommands configKey:" + configKey);
    //debug("extractCommands commandsObj:" + JSON.stringify(commandsList) );
    if (commandsList === undefined) { commandsList = []; }
    var keyList = [];


    //debug("commands list=" + JSON.stringify(commandsList) );
    for (let i = 0; i < commandsList.length; i++) {
      const cmdObj = commandsList[i];
      const cmdTypeKey = cmdObj.type;
      debug("cmdTypeKey: " + cmdTypeKey);
      ...
      //debug("command item=" + JSON.stringify(commands[cmdKey]) );
...
function assignClones() {
  // review all commands for cloneVal sources and cloneVal dependents
   ...
   // identify commands dependent on cloneValues
    ...
    debug("=== Identify clones using cloneValues from user ion model.");
...
function extractConfigs(groupKey,configsObj) {
    ...
    debug("extractConfigs groupKey:" + groupKey);
    //debug("extractConfigs configsObj:" + JSON.stringify(configsObj) );
    ...
    for (var configType in configsObj) {   // object names are configTypes
      //debug("loop for configType:" + configType);
      ...
      //debug("Config got cmdKeys: " + cmdKeyList);
...
function checkModel() {
  // check model for errors
    ...
    for (var nodeKey in nodes) {
      ...
      debug("checking commands for version seq: " + ionVerSeqNo.toString())
      ...
      debug("+++ conf count: " + nconfs.length);
      // node configs loop
      for (let k=0; k<nconfs.length; k++) {
        ...
        debug("+++ checking reqd cmds: " + missed.toString());
        ...
        for (let n=0; n<configObj.cmdKeys.length; n++) { 
          ...
          debug("checking command type: " + cmdTypeKey + " " );
        ...
        debug("***** cmdCounts " + JSON.stringify(cmdCounts));
        for (var cntKey in cmdCounts) {
          if (cmdCounts[cntKey] > 1) {
            debug("***** " + cntKey + " count =" + cmdCounts[cntKey].toString() );
      ...
      debug("***** ductFlags " + JSON.stringify(ductFlags));
      debug("***** protFlags " + JSON.stringify(protFlags));
...
function getIonVerSeqNo(nodeKey) {
    debug("getIonVerSeqNo for:" + nodeKey);
    ...
    if (nodeKey in nodes) {
      ...
      debug("??? graph: " + JSON.stringify(graph));
      ver = graph.ionVersion;
    }
    debug("???nodeKey: " + nodeKey + " ionVer: "+ ver);
...
function makeCmdLine(cmdTypeKey,cmdParams) {
  // builds ION command text (commands file format)
    // debug("makeCmdLine cmdTypeKey: " + cmdTypeKey + "  Params : " + JSON.stringify(cmdParams));
    const targets = [ "[1]", "[2]", "[3]", "[4]", "[5]", "[6]", "[7]", "[8]", "[9]" ];
    var cmdPattern = patterns[cmdTypeKey];
    let cmdType = cmdTypes[cmdTypeKey];
    //debug("cmdType: " + JSON.stringify(cmdType));
    for (var i = 0; i < cmdParams.length; i++) {
      //debug("makeCmdLine Pattern: " + cmdPattern + " tgt: " + cmdTypeKey + "  " + targets[i] + "  " + cmdParams[i]);
    ...
    //debug("cmd Type pattern " + cmdTypeKey + " " + cmd);
    return cmdPattern;
  };
// Automatically extracted from source file ../../editor/src/IonModel.jsx
function makeCmdLines(configObj) {
    ...
    //debug("makeConfigElem configType:" + JSON.stringify(configType));
    const modelName = ion.name;
    ...
    //debug("makeCmdLines commands: " + Object.keys(commands).length); 
    ...
    for (var i=0; i<cmdKeys.length; i++) {
      ...
      //debug("makeCmdLines i: " + i + " cmdKey: " + cmdKey);
      const cmd = commands[cmdKey];
      const cmdTypeKey = cmd.typeKey;
      //debug("makeCmdLines cmdTypeKey: " + cmdTypeKey);
      const cmdType = cmdTypes[cmdTypeKey];
...
function makeStartLines(nodeKey) {
  // build the start script text lines for a node
    ...
    debug("makeStartLines sortConfigs: " + JSON.stringify(configFiles));
    for (var j=0; j<configFiles.length; j++) {
      let configObj = configFiles[j];
      let configKey = configObj.id;
      debug("makeStartLines configKey: " + configKey);
...
function makeParamNote(pTypeKey,pIdx,paramVal) {
  // build ION parameter annotation (commands file format)
    ...
    //debug("makeParamNote: " + pTypeKey + " " + pIdx + " " + paramVal);
    ...
    if (sels) {
      //debug("makeParamNote sels: " + JSON.stringify(sels));
    ...
    //debug("param Note " + pTypeKey + " " + note);
    return note;
  }
